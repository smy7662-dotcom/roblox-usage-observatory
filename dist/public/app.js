...
function renderChart() {
  ...
  ctx.fillStyle='#89909b'; const labelEvery=Math.max(1,Math.floor(points.length/7)); points.forEach((p,i)=>{if(i%labelEvery===0||i===points.length-1){const d=parseDate(p.date);const year=d.getUTCFullYear();const month=String(d.getUTCMonth()+1).padStart(2,'0');const day=String(d.getUTCDate()).padStart(2,'0');const hour=String(d.getUTCHours()).padStart(2,'0');const label=state.frequency==='monthly'?`${year}-${month}`:state.frequency==='hourly'?`${year}-${month}-${day} ${hour}:00`:`${year}-${month}-${day}`;const width=ctx.measureText(label).width;ctx.fillText(label,Math.max(pad.l,Math.min(w-pad.r-width,x(i)-width/2)),h-15);}});
...
$$('.rail-card').forEach(button=>button.addEventListener('click',()=>{const target=button.dataset.module==='games'?'#gamesCard':'#chartCard';$$('.rail-card').forEach(item=>item.classList.toggle('active',item===button));document.querySelector(target)?.scrollIntoView({behavior:'smooth',block:'start'});}));
